 ...
 *
 * `data-mlady` na řádku je jediné, co z označení věku potřebuje filtr —
 * schovávání pak obstará CSS a tabulka může zůstat celá na serveru.
 */
export function SeznamKandidatu({ strana }: { strana: StranaNaKandidatce }) {
  ...
  const mladych = strana.kandidati.filter((k) => jeMladyKandidat(k.vek)).length
  const skrtnute = strana.skrtnutePozice ?? []

  return (
    <>
      <FiltrKandidatky mladych={mladych} celkem={strana.kandidati.length + skrtnute.length}>
          ...
          <table className="w-full min-w-[38rem] border-collapse text-left text-sm">
            ...
            <tbody>
              ...
              {skrtnute.map((poradi) => (
                <tr
                  key={`skrtnuto-${poradi}`}
                  /* Prázdná pozice není osoba, takže do výběru podle věku nepatří. */
                  data-mlady="ne"
                ...
                >
                  ...
                  <td className="py-2 text-seda-uredni" colSpan={4}>
                    Pozice zůstala volná — kandidát byl při registraci škrtnut a registrační úřad ji
                    neobsadil.
                  </td>
                </tr>
              ))}
